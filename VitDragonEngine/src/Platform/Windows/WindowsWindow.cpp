...
namespace VitDragonEngine{
	...
	static void GLFWErrorCallback( int err, const char *description ){
		VDE_CORE_ERROR( "GLFW Error ({0}): {1}", err, description );
	}
	...
	void WindowsWindow::Init( const WindowProps &props ){
		...
		VDE_CORE_INFO( "Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height );

		if( !s_GLFWInitialized ){
			int succes = glfwInit();
			VDE_CORE_ASSERT( success, "Could not initialize GLFW" );
